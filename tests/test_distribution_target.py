...
def test_interpreter(
    ...
    # type: (...) -> None
    assert DistributionTargets().interpreter is None
    assert py27 == DistributionTargets(interpreters=(py27,)).interpreter
    assert py27 == DistributionTargets(interpreters=(py27, current_interpreter)).interpreter
...
def test_unique_targets(
    ...
    # type: (...) -> None
    assert (
        OrderedSet([DistributionTarget.current()]) == DistributionTargets().unique_targets()
    ), "Expected the default TargetConfiguration to produce the current interpreter."

    assert (
        OrderedSet([DistributionTarget.current()])
        == DistributionTargets(platforms=(None,)).unique_targets()
    ), (
    ...
    assert (
        ...
        == DistributionTargets(interpreters=(py27,), platforms=(None,)).unique_targets()
    ), (
    ...
    assert (
        ...
        == DistributionTargets(platforms=(current_platform,)).unique_targets()
    )

    assert (
        OrderedSet(DistributionTarget.for_interpreter(i) for i in (py27, py37, py310))
        == DistributionTargets(interpreters=(py27, py37, py310)).unique_targets()
    )
