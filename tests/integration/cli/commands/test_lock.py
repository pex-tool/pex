...
def test_update_add_impossible(
    ...
    assert not result.output
    assert (
        "Added conflicting-certifi-requirement 1.2.3 to lock generated by "
        "cp38-cp38-manylinux_2_33_x86_64.\n" == result.error
    )
...
def test_export(tmp_path):
    ...
    lock_file = tmp_path / "requirements.lock.json"
    run_pex3("lock", "create", "requests", "-o", lock_file.as_posix()).assert_success()
    # Add pants style comments to lock file:
    lock_file.write_text(PANTS_STYLE_COMMENTS + lock_file.read_text())
    requirements_file = tmp_path / "requirements.lock.txt"
    run_pex3(
        "lock", "export", "-o", requirements_file.as_posix(), lock_file.as_posix()
    ).assert_success()

    # We should get back the same lock given a lock as input mod comments (in particular the via
    # comment line which is sensitive to the source of the requirements)
    result = run_pex3("lock", "create", "-r", requirements_file.as_posix())
    result.assert_success()
    assert normalize_lockfile(json_codec.load(lock_file)) == normalize_lockfile(
        json_codec.loads(result.output)
    )
