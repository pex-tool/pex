...
def run_lock_update(
    ...
    **env  # type: Optional[str]
    ...
    return run_pex3("lock", "update", "--python", py38, *args, **env)


def test_update_noop(lock_file_path):
    # type: (str) -> None
    result = run_lock_update("-p", "urllib3==1.25.11", lock_file_path)
    result.assert_success()
...
def test_update_noop_dry_run(lock_file_path):
    # type: (str) -> None
    result = run_lock_update("-n", "-p", "urllib3==1.25.11", lock_file_path)
    result.assert_success()
    assert (
...
def test_update_targeted_upgrade_dry_run(lock_file_path):
    # type: (str) -> None
    result = run_lock_update("-n", "-p", "urllib3<1.26.7", lock_file_path)
    result.assert_success()
    assert (
...
def test_update_targeted_downgrade(lock_file_path):
    # type: (str) -> None
    result = run_lock_update("-p", "urllib3<1.25", lock_file_path)
    result.assert_success()
    assert not result.output
    assert (
...
def test_update_targeted_closure_shrink(lock_file_path):
    ...
    result = run_lock_update("-p", "requests==2.0.0", lock_file_path)
    result.assert_success()
    lock_file = lockfile.load(lock_file_path)
    assert 1 == len(lock_file.locked_resolves)
    locked_resolve = lock_file.locked_resolves[0]
...
def test_update_targeted_impossible(
    ...
    # type: (...) -> None
    result = run_lock_update("-p", "urllib3<1.16", lock_file_path)
    ...
    result = run_lock_update("-p", "urllib3<1.16", updated_lock_file_path)
    result.assert_success()
    assert not result.output
    assert (
