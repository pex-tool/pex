    ...
    run_sync(
        ...
        expected_error_re=re_exact(
            dedent(
                ...
                """
            ).format(platform=locked_resolve.target_platform)
        ),
    )
    assert_lock(
        initial_lock,
        ...
        expected_error_re=re_exact(
            dedent(
                ...
                Updates to lock input requirements:
                  Updated 'foo' to 'foo>1'
                """
            ).format(platform=lock_file.locked_resolves[0].target_platform)
        ),
    )
    lock_file = json_codec.load(initial_lock, path_mappings=path_mappings)
    assert_lock(
...
def test_sync_venv_run(
    ...
    assert (
        dedent(
            ...
            """
        ).format(platform=lockfile.locked_resolves[0].target_platform)
        == result.error
    )
    ...
    run_sync(
        *(
        ...
        expected_error_re=r".*No updates for lock generated by {platform}\..*".format(
            platform=re.escape(str(lock_file.locked_resolves[0].target_platform))
        ),
        re_flags=re.DOTALL,
    )
    assert_lock(lock, path_mappings, expected_pins=[pin("cowsay", "5.0")])
