...
def create_whl(
    ...
    with open_zip(wheel_path, "w", compression=ZIP_DEFLATED if compress else ZIP_STORED) as zip_fp:
        if use_system_time and wheel_to_create.zip_metadata:
            ...
            # These are not present in whls, they are generated by the installation process.
            paths_to_skip = (
                wheel_to_create.wheel.metadata_path("INSTALLER"),
                wheel_to_create.wheel.metadata_path("REQUESTED"),
                wheel_to_create.wheel.metadata_path("direct_url.json"),
            )
            for installed_file in _read_record_lines(record_data.decode("utf-8").splitlines()):
                ...
                if use_system_time:
                    ...
                    zip_fp.write_deterministic(src, path, compress=compress)
    return wheel_path
