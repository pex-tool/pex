...
class bdist_pex(Command):  # noqa
  ...
  def run(self):
    ...
    args = [sys.executable, '-sE', '-m', 'pex.bin.pex', package_dir] + self.pex_args
    ...
    for script_name, target in pex_specs:
      ...
      result = Executor.open_process(cmd).wait()
      if result != 0:
        die('Failed to create pex via {}'.format(' '.join(cmd)), result)
