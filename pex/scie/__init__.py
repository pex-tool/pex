...
def register_options(parser):
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "all distributions contained in the PEX, use `*:*`. To just pick all the console "
            "scripts from a particular project name's distributions in the PEX, use "
            "`<project name>:*`; e.g.: `ansible-core:*`. To exclude all the console scripts from a "
            "project, prefix with a `!`; e.g.: `*:*,!ansible-core:*` selects all console scripts "
            "except those provided by the `ansible-core` project. To select an individual console "
            "script, just use its name or prefix the name with `!` to exclude that individual "
            "console script. To specify an arbitrary entry point in a module contained within one "
            "of the distributions in the PEX, use a string of the form "
            "`<name>:<module>(:<function>)`; e.g.: 'run-baz:foo.bar:baz' to execute the `baz` "
            "function in the `foo.bar` module as the entry point named `run-baz`. Multiple entry "
            "points can be specified at once using a comma-separated list or the option can be "
            "specified multiple times. A BusyBox scie has no default entrypoint; instead, when "
            "run, it inspects argv0; if that matches one of its embedded entry points, it runs "
            "that entry point; if not, it lists all available entrypoints for you to pick from. To "
            "run a given entry point, you specify it as the first argument and all other arguments "
            "after that are forwarded to that entry point. BusyBox PEX scies allow you to install "
            "all their contained entry points into a given directory. For more information, run "
            "`SCIE=help <your PEX scie>` and review the `install` command help."
        ),
    )
    parser.add_argument(
        "--scie-platform",
