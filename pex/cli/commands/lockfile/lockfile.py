...
class _RankedLock(object):
    ...
    def rank(
        ...
        To help select which locked resolve to use, ranking gives an absolute score to each locked
        resolve that is the sum of the score of each locked requirement in the resolve. Each locked
        requirement is, in turn, scored by its best matching artifact. Artifacts are scored as
        follows:
